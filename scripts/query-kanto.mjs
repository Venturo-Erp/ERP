...
async function queryKantoAttractions() {
  ...
  const { data: cities } = await supabase
    ...
    .eq('country_id', 'japan')
    .or('name.like.%東京%,name.like.%橫濱%,name.like.%鎌倉%,name.like.%川越%,region_id.like.%kanto%');
  
  console.log('關東地區城市：', cities?.map(c => c.name));
  ...
  const needRewrite = attractions.filter(a => 
    !a.description || 
    a.description.length < 50 || 
    a.description.includes('熱門景點')
  );
