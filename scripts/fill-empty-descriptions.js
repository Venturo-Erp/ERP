...
const BLACKLIST = [
  '大象', '老虎', '獅子', '黑熊', '動物表演', '鬥牛', '鬥雞',
  'elephant', 'tiger', 'lion', 'bear', 'animal show'
];
...
async function processBatch(table, items, batchSize = 10) {
  ...
  for (let i = 0; i < items.length; i += batchSize) {
    ...
    for (const item of batch) {
      ...
      const description = await generateDescription(item.name, item.country_id, item.city_id, searchResults);
      if (!description) {
