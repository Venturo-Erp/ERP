...
const migrationSQL = readFileSync('supabase/migrations/20260318_extend_tour_requests_for_local_quotes.sql', 'utf-8')
...
try {
  ...
  // 驗證新表
  const tableResponse = await fetch(`https://api.supabase.com/v1/projects/${projectRef}/database/query`, {
    method: 'POST',
    headers: {
      'Authorization': `Bearer ${accessToken}`,
      'Content-Type': 'application/json',
