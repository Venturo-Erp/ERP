...
export function DriverInfoForm({ itemId }: { itemId: string }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      {/* 錯誤訊息 */}
      {error && (
        <div className="p-3 bg-morandi-red/10 border border-morandi-red/30 rounded-lg text-morandi-red text-sm">
          {error}
        </div>
      )}
      ...
      <button
        ...
        className="w-full py-4 bg-morandi-gold text-white font-bold rounded-lg hover:bg-morandi-gold-hover disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
      >
