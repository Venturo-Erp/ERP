...
import { ActivityQuoteForm } from '../ActivityQuoteForm'
...
export default async function ActivityQuotePage({
  params,
}: {
        ...
        <div className="bg-white rounded-xl shadow-lg overflow-hidden border border-border">
          ...
          <div className="bg-gradient-to-r from-morandi-gold to-morandi-gold-hover px-6 py-4 text-white">
            ...
            <div className="mt-2 flex items-center gap-6 text-sm opacity-90">
              <span>活動：{request.supplier_name}</span>
          ...
          <div className="p-6 space-y-6">
            {/* 活動需求 */}
            <div>
              <h3 className="font-semibold text-morandi-gold mb-3">活動需求</h3>
              <div className="bg-morandi-container border border-border rounded-lg p-4">
                {requestItems.length > 0 ? (
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-border">
                        <th className="text-left py-2">活動名稱</th>
                        <th className="text-left py-2">活動時間</th>
                    ...
                    <tbody>
                      {requestItems.map((item: Record<string, unknown>, idx: number) => (
                        <tr key={idx} className="border-b border-border">
                          <td className="py-2">{(item.activity_name as string) || (item.name as string) || '—'}</td>
                          <td className="py-2">{(item.activity_time as string) || '—'}</td>
                  ...
                  <p className="text-sm text-morandi-secondary">無活動需求</p>
                )}
              </div>
            </div>
