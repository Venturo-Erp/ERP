...
export default function OvertimePage() {
  ...
  const url = `/api/hr/overtime-requests?scope=workspace${filter === 'pending' ? '&status=pending' : ''}`
  const { data, mutate, isLoading } = useSWR<OvertimeRow[]>(url, fetcher, {
  ...
  const handle = async (id: string, action: 'approve' | 'reject') => {
    ...
    setBusy(id)
    try {
      const res = await fetch(`/api/hr/overtime-requests/${id}/${action}`, {
        method: 'POST',
    ...
    <div className="space-y-4">
      ...
      {isLoading ? (
        ...
        <div className="rounded-xl border border-border/60 bg-card/50 p-8 text-center text-sm text-morandi-muted">
          {filter === 'pending' ? '目前沒有待審核加班申請' : '無加班紀錄'}
        </div>
      ) : (
                ...
                <div className="flex-1 min-w-0 space-y-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <span className="text-[10px] px-1.5 py-0.5 rounded bg-morandi-container/50 text-morandi-secondary">
                      {TYPE_LABEL[r.overtime_type] ?? r.overtime_type}
                    </span>
                  </div>
                  <div className="text-xs text-morandi-secondary">
                    {r.date} {r.start_time}–{r.end_time} · {r.hours} 小時
                  </div>
                  {r.reason && <div className="text-xs text-morandi-muted">{r.reason}</div>}
                  {r.reject_reason && (
