  ...
  const initDefaults = async () => {
    ...
    const defaults = [
      {
        ...
        calc_config: { rate: 0.115, employee_share: 0.2 },
        sort_order: 1,
      },
      {
        ...
        name: '全民健保',
        calc_method: 'percentage',
        calc_config: { rate: 0.0517, employee_share: 0.3 },
        sort_order: 2,
      },
      {
        code: 'pension',
        name: '勞工退休金',
        calc_method: 'percentage',
        calc_config: { rate: 0.06, employee_share: 0 },
    ...
    for (const d of defaults) {
      await supabase.from('payroll_deduction_types' as never).upsert(
        ...
        { onConflict: 'workspace_id,code' } as never
      )
    }
  ...
  const handleSaveDeduction = async () => {
    ...
    try {
      ...
      const data = {
        ...
        calc_method: dMethod,
        calc_config: config,
        is_employer_paid: dEmployerPaid,
        is_active: true,
  ...
  const handleSaveAllowance = async () => {
    ...
    try {
      const data = {
        ...
        code: aCode,
        name: aName,
        default_amount: aAmount,
        is_taxable: aTaxable,
        is_active: true,
        sort_order: allowances.length,
      }
      if (editingAllowance) {
        await supabase
          .from('payroll_allowance_types' as never)
    ...
    >
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] px-1.5 py-0.5 rounded bg-morandi-container text-morandi-muted font-mono">
                        {d.code}
                      </span>
                      {d.is_employer_paid && (
                        <span className="text-[10px] px-1.5 py-0.5 rounded bg-status-info-bg text-status-info">
                          雇主負擔
                        </span>
                      )}
                    </div>
                    <p className="text-xs text-morandi-muted mt-0.5">
                      ...
                      {d.calc_method === 'percentage' && d.calc_config?.rate
                        ? ` · ${(Number(d.calc_config.rate) * 100).toFixed(2)}%`
                        : ''}
                      {d.calc_method === 'fixed' && d.calc_config?.amount
                        ? ` · $${d.calc_config.amount}`
                        : ''}
                    </p>
                  </div>
                  <div className="flex gap-1">
                    <button
                      onClick={() => {
                        ...
                        setDCode(d.code)
                        setDName(d.name)
                        setDMethod(d.calc_method)
                        setDAmount(Number(d.calc_config?.amount) || 0)
                        setDRate((Number(d.calc_config?.rate) || 0) * 100)
                        setDEmployerPaid(d.is_employer_paid)
                        setShowDeductionDialog(true)
                      }}
                      className="p-1.5 text-morandi-secondary hover:text-morandi-gold rounded"
                    >
                      <Edit2 size={14} />
                    </button>
                    <button
                      onClick={() => deleteDeduction(d.id)}
                      className="p-1.5 text-morandi-secondary hover:text-morandi-red rounded"
                    >
                      <Trash2 size={14} />
                    </button>
                  </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      {!a.is_taxable && (
                        <span className="text-[10px] px-1.5 py-0.5 rounded bg-morandi-green/10 text-morandi-green">
                          免稅
                        </span>
                      )}
                    </div>
                    <p className="text-xs text-morandi-muted mt-0.5">
                      預設金額 ${a.default_amount.toLocaleString()}
                    </p>
                  </div>
                  <div className="flex gap-1">
                    <button
                      onClick={() => {
                        ...
                        setACode(a.code)
                        setAName(a.name)
                        setAAmount(a.default_amount)
                        setATaxable(a.is_taxable)
                        setShowAllowanceDialog(true)
                      }}
                      className="p-1.5 text-morandi-secondary hover:text-morandi-gold rounded"
                    >
      ...
      <Dialog open={showDeductionDialog} onOpenChange={setShowDeductionDialog}>
        <DialogContent level={1} className="max-w-md">
          <DialogHeader>
            <DialogTitle>{editingDeduction ? '編輯扣款項目' : '新增扣款項目'}</DialogTitle>
          </DialogHeader>
            ...
            {dMethod === 'percentage' && (
              <div>
                ...
                <Input
                  ...
                  step="0.01"
                  value={dRate}
                  onChange={e => setDRate(Number(e.target.value))}
                  className="mt-1"
                />
              </div>
            )}
      ...
      <Dialog open={showAllowanceDialog} onOpenChange={setShowAllowanceDialog}>
        <DialogContent level={1} className="max-w-md">
          <DialogHeader>
            <DialogTitle>{editingAllowance ? '編輯津貼項目' : '新增津貼項目'}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 pt-2">
            <div className="grid grid-cols-2 gap-4">
              <div>
