...
export const employeeEntity = createEntityHook<Employee>('employees', {
  list: {
    select:
      'id,employee_number,display_name,chinese_name,english_name,email,avatar,avatar_url,birth_date,id_number,employee_type,status,is_active,roles,monthly_salary,supabase_user_id,user_id,must_change_password,last_login_at,workspace_id,created_at,updated_at,updated_by',
    orderBy: { column: 'employee_number', ascending: true },
  },
  slim: {
    select:
      'id,employee_number,display_name,chinese_name,english_name,email,status,roles,workspace_id,employee_type,avatar',
  },
  detail: { select: '*' },
