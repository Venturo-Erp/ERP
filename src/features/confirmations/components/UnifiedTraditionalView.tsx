  ...
  return (
    <div className="bg-white flex flex-col min-h-full">
      ...
      <table className="w-full border-collapse text-sm mb-6">
        <tbody>
          <tr>
            <td className="border border-morandi-muted px-3 py-2 bg-background font-semibold w-[90px]">
              致
            </td>
            ...
            <td className="border border-morandi-muted px-3 py-2">
              <input
                type="text"
                value={contact || ''}
                onChange={e => onInfoChange?.('contact', e.target.value)}
                placeholder=""
          ...
          <tr>
            <td className="border border-morandi-muted px-3 py-2 bg-background font-semibold">
              電話
            </td>
            <td className="border border-morandi-muted px-3 py-2">
              <input
            ...
            <td className="border border-morandi-muted px-3 py-2">
              <input
                type="text"
                value={fax || ''}
                onChange={e => onInfoChange?.('fax', e.target.value)}
                placeholder=""
                className="w-full bg-transparent outline-none focus:ring-0"
              />
            </td>
      ...
      <div className="footer-grid mt-6 grid grid-cols-2 gap-6 border-t border-morandi-muted pt-4">
        ...
        <div className="text-sm space-y-1">
          <div>
            <span className="font-medium text-morandi-secondary">公司名稱：</span>
            {ws.name || COMPANY_NAME}
          </div>
          <div>
            <span className="font-medium text-morandi-secondary">公司電話：</span>
            {ws.phone || ''}
          </div>
          <div>
          ...
          <div>
            <span className="font-medium text-morandi-secondary">助理：</span>
          </div>
        </div>
        {/* 右邊：供應商簽回 */}
        <div className="text-sm space-y-1">
          <div>
            <span className="font-medium text-morandi-secondary">供應商確認：</span>
          </div>
      ...
      <table className="w-full border-collapse text-sm table-fixed">
        <thead>
          <tr className="bg-background">
            <th
              className="border border-morandi-muted px-3 py-2 text-left font-semibold"
              style={{ width: '110px' }}
            >
              入住日期
            </th>
            <th className="border border-morandi-muted px-3 py-2 text-left font-semibold">
              需求房型
            </th>
            <th
              className="border border-morandi-muted px-3 py-2 text-left font-semibold"
            ...
            <th
              className="border border-morandi-muted px-3 py-2 text-center font-semibold"
              style={{ width: '70px' }}
            >
              數量
            </th>
            <th
              className="border border-morandi-muted px-3 py-2 text-right font-semibold"
              style={{ width: '120px' }}
            >
              報價
            </th>
        ...
        <tbody>
          {items.map((item, idx) => (
            <tr key={idx}>
              ...
              <td className="border border-morandi-muted px-3 py-2 text-center">
          ...
          {items.length === 0 && (
            <tr>
              <td
                colSpan={5}
                className="border border-morandi-muted px-3 py-4 text-center text-morandi-muted"
              >
                無資料
              </td>
            </tr>
            ...
            <tr key={idx}>
              ...
              <td className="border border-morandi-muted px-1 py-1">
                <input
                  ...
                  placeholder=""
                  className="w-full px-2 py-1 bg-transparent outline-none focus:ring-1 focus:ring-morandi-gold rounded"
                />
              </td>
              <td className="border border-morandi-muted px-3 py-2">
                {String(item.venue || item.title || '')}
              </td>
              <td className="border border-morandi-muted px-1 py-1">
                <input
                  type="number"
                  ...
                  className="w-full px-2 py-1 bg-transparent outline-none focus:ring-1 focus:ring-morandi-gold rounded text-center"
                />
              </td>
              <td className="border border-morandi-muted px-3 py-2 text-right"></td>
            </tr>
        ...
        <tbody>
          {items.map((item, idx) => (
            <tr key={idx} className={idx % 2 === 0 ? 'bg-white' : 'bg-morandi-red/10'}>
              ...
              <td className="border border-morandi-red/30 px-3 py-2">
                {String(item.quantity || '—')}
              </td>
