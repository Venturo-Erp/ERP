...
async function fetchTourRequests(tourId: string): Promise<TourRequest[]> {
  const { data, error } = await supabase
    .from('tour_requests')
    .select(
      'id, code, tour_id, workspace_id, request_type, status, supplier_name, supplier_id, items, note, sent_at, replied_at, replied_by, confirmed_at, confirmed_by, recipient_workspace_id, target_workspace_id, hidden, created_at, created_by, updated_at, updated_by'
    )
    .eq('tour_id', tourId)
