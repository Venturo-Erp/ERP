  ...
  const loadData = async () => {
    ...
    try {
      ...
      const { data: magic } = await supabase
        .from('magic_library')
        .select(
          'id, name, category, description, source_type, layer, github_url, official_url, current_version, latest_version, update_status, check_frequency, last_checked_at, workspace_id, created_at, updated_at'
        )
        .order('category')
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="flex-1 overflow-hidden flex flex-col">
        <div className="flex-1 overflow-auto">
          {activeTab === 'magic' && <MagicLibraryView items={magicItems} loading={loading} />}
          {activeTab === 'bots' && <BotManagementView bots={bots} loading={loading} />}
