...
- Wave 2 · B5：Database 9 頁 admin layout guard
...
3. **Database layout 沒驗證非 admin 員工影響**（William 自己會登非 admin 帳號測 `/database/attractions`）
...
## 2026-04-21 07:47 — Wave 2 Batch 5：Database 管理頁 layout admin guard

### 範圍
新增 `src/app/(main)/database/layout.tsx`、cover 9 個子路由（archive-management / attractions / company-assets / constants / fleet / suppliers / tour-leaders / transportation-rates / workspaces）— 全部 admin-only。
...
- Wave 2 Batch 3（HR admin 頁、需 William 確認 scope）
...
### Wave 2 Batch 1 · Finance 6 頁 admin guard
...
- 新建 `src/app/(main)/accounting/layout.tsx` admin-only
...
- 如果沒修、useTabPermissions 會把 admin 當一般用戶擋
...
2. **`/tools/reset-db` 加 admin guard**
   - `usePermissions` 取 isAdmin、非 admin 顯示「需要管理員權限」畫面
...
- William 確認後執行 Wave 0（3 項 P0、S 工期、code only、零 DB）：
  1. `'current_user'` literal × 2（travel-invoice void）
  2. `/tools/reset-db` 無 admin guard
