...
**一句話**：把「系統主管」從「bypass key」變回「權限預設多的 role」、不動任何其他東西。
...
| ✅ DB migration：給系統主管 role 的 `workspace_roles` 掛預設全權（避免拔掉短路後 系統主管 斷事）                                           | 是           | 這是「讓系統主管 不爆」的補償、P001 的必要配套                         |
...
| ❌ 把 40+ 個 `isAdmin` 引用 grep 全拔                                               | **不該做**          | 60+ 檔案裡大多數 `isAdmin` 是 UI **顯示**差異（系統主管按鈕、系統主管面板入口）、不是**權限決策**。拔錯會誤傷 sidebar / widget 顯示邏輯 |
...
- 範圍：`src/stores/auth-store.ts:249` 拔短路 + DB migration 給系統主管 role 預填全權限
- 預估：1 檔 TS（1 行改動）+ 1 檔 migration（~20 行 SQL）
- Diff size：< 30 行
- 驗收：系統主管仍能正常操作（因為 DB 層預填了）、沒有管理員資格 role 的 permission 檢查生效
...
- 範圍：`finance/payments/page.tsx:211`（整頁 系統主管 擋）+ `tour-itinerary-tab.tsx:80-91`（canEditDatabase）
- 預估：2 檔 × ~2 行 = ~4 行
- Diff size：< 20 行
- 驗收：有 `finance.payments.view` 權限的沒有系統主管資格 能進頁 / 有 `database` 權限的沒有系統主管資格 能編輯資料庫
...
1. **權限決策點**（要改）：約 8-10 個、都在本次 3 個 PR 涵蓋範圍
   ...
   - **其他 /finance/_ + /accounting/_ 頁**：盤點是否同型整頁擋、同型處理、但分到對應路由 PR 不併入 P001

2. **UI 顯示差異**（**不改**）：約 50+ 個、例如：
   - sidebar 顯示系統主管專屬連結
   - dashboard widget 只給系統主管看
   - settings 頁「系統主管工具」區塊
