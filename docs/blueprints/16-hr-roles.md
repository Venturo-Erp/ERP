...
stateDiagram-v2
    ...
    Saved2 --> SelectRole
    SelectRole --> Deleted: 按刪除（非 admin）
    Deleted --> Idle
```
...
| `DELETE /api/roles/[id]` | 刪職務（非 admin）|
...
違反 INVARIANT：**INV-A02 Settings 必 admin**（本頁屬 HR 管理、同理必 admin）。
...
### ADR-R3 · 此頁違反 INV-A02（無 admin guard）
**現狀**：任何登入員工都能打開 `/hr/roles` 看/改權限
**違反**：INV-A02 Settings 必 admin
...
### ❌ 不要刪 `is_admin` 角色
**規則**：UI 已擋、API 層也必須拒絕（若 admin 被刪、沒人能管系統）
...
| 2 | 無 admin guard、任何員工可改權限 | INV-A02 | 🔴 P0、🟢 Stage C 可修 |
...
- #2 補 admin guard（page 開頭 `if (!isAdmin) redirect('/unauthorized')`）
