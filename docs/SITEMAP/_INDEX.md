...
Last updated：2026-04-22（候選原則 4 升正式、首驗 /tours 加候選原則 5-8、/tours 命中多條既有共通問題 / 首建 `_PATTERN_MAP.md` 收斂全部跨路由病症為 15 條 pattern）
...
|---|---|---|---|---|
| [/login](./login.md) | 員工登入（未來 SaaS 多租戶） | 2026-04-22 **v2.0** | admin 萬能通行證 + middleware 讓密碼重設等 API 免登入 + sync-employee 可跨租戶綁帳號 + 登入後首頁會被 28 張 FORCE RLS 表擋住 | 違反原則 1 + 3 |
...
| 路由 | 可能中的 pattern | 緊急度 |
|---|---|---|
| [/hr](./hr.md) | 欄位識別符多重（employees 三個登入 ID）/ FORCE RLS + service_role / middleware 公開清單寬鬆 | 🟡 次頁驗完再回頭 |
| [/finance/payments](./finance_payments.md) | 候選原則 8（快速入口 ≠ 獨立資料）確認是否命中、payments 該不該被視為訂單頁延伸 | 🟡 候選原則拍板後 |
...
### 🟡 RLS policy `USING: true`（完全無過濾）（v2.0 新增）
**樣態**：table RLS 開了、但 policy 條件是 `true`（任何登入用戶都能讀全部）。看起來有保護、實際沒有。

- **已確認命中**：`employee_permission_overrides`
