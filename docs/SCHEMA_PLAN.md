...
| 表 | 行數 | RLS | 說明 |
|----|------|-----|------|
| `channels` | 24 | ✅ | 頻道（員工聊天群） |
| `channel_groups` | 1 | ✅ | 頻道群組 |
| `channel_members` | 22 | ✅ | 頻道成員 |
| `messages` | 0 | ✅ | 頻道訊息（自我引用支援討論串、reactions/attachments jsonb） |

**狀態**：基礎建設完整、零實際使用。
- 24 個頻道有人開、22 員工已加入、Realtime 訂閱寫好
- 0 筆訊息（沒人傳）
- UI 完整：`src/app/(main)/channel/` + `src/components/workspace/channel-chat/`（14 個 hook）

**凍結原因**（William 2026-05-02 拍板）：
- 業務優先級不足、效益相對小
- 強制員工從 LINE 轉換到內部聊天難度高、不是首要解決
- 有替代方案先頂著

**未來解凍方向**：
- LINE 開始收費 / 檔案保存問題逼近時、需要做 Slack 替代品
- 整合 cron / workflow、做事件驅動的頻道通知（自動化提醒）
- 解凍時依憲法重新審視 schema、UI 拆模組、不繼承現有實作半成品

**凍結期間禁止**（憲法 Section 10）：
- ❌ 對 4 表加新欄位
- ❌ 對 `/channel` UI 加新功能
- ❌ 對 `chat-store` / `channel-chat/*` hooks 重構或擴充
- ✅ 允許：純 bug fix、跟著全站 type-check / lint 一起改的機械式調整
