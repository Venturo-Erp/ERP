...
const { data: tourData } = await supabase.from('tours').select('custom_cost_fields') // ← JSON 陣列

// 成員的值存在 order_members.custom_costs (JSON key-value)
const { data: membersWithCosts } = await supabase.from('order_members').select('id, custom_costs')
