...
- **級聯後果**：workspace_roles / employees / tour_* / orders / receipts 全部跟著刪（CASCADE FK）
...
**本週（P1）**：
4. `employee_permission_overrides` 加 workspace_id + 改 policy（blocking migration）
5. amadeus_totp_secret 所有 SELECT 加欄位白名單（不用 `*`）

**下週（P2）**：
6. validate-login 補讀 workspaces.setup_state / premium_enabled / enabled_tour_categories / max_employees
7. workspaces AFTER INSERT trigger 加 exception handler

**規劃清理（P3）**：
8. 清除 employees.permissions / roles 舊欄位（cleanup-council 任務）
