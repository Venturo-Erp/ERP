...
## 🎨 Status Pill SSOT 規則（防散兵）

> 2026-04-25 新增。事件回顧：原本 status-config.ts 是統一機制、但隨時間推移、各 component 自己手刻 `<span class="bg-morandi-X/10 text-morandi-X rounded-full">` 渲染 status，導致：
> - `/finance/requests` 的「待處理」用 `bg-morandi-gold` 純色立體、其他狀態 soft pill 平面 → 視覺不一致
> - `/finance/payments` 用 StatusCell + Badge（有 border + font-semibold）、跟 StatusBadge 不一樣
> - HR / 合約 / 景點 / 簽到 各自手刻、毫無統一
>
> 已於 2026-04-25 commit a93ec9c7d / 3f5a601b6 / c73b79643 / 後續批次統一。

### 唯二准用 component

```tsx
// 簡單場景（自己決定 tone + label）
import { StatusBadge } from '@/components/ui/status-badge'
<StatusBadge tone="pending" label="待處理" />

// 走 status-config 集中 mapping（已知 entity + status string）
import { StatusCell } from '@/components/table-cells'
<StatusCell type="receipt" status={r.status} />
```

### 6 種 tone（StatusBadge）

| tone | 色 | 用途 |
|---|---|---|
| `pending` | 柔和灰 | 待處理 / 待確認 / 待送件 |
| `info` | 柔和藍 | 已確認 / 進行中 / 處理中 |
| `success` | 柔和綠 | 已出帳 / 已付 / 已完成 / 已驗證 |
| `warning` | 柔和黃 | 警告 / 過期 / 試用期 |
| `danger` | 柔和紅 | 已取消 / 失敗 / 已終止 |
| `neutral` | 中性 | 預設 / 未分類 / 在職 |

### 視覺規格（共用）

- `inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium whitespace-nowrap`
- 半透明背 `bg-X/15` + 同色字 `text-X`
- **不准** shadow / gradient / border / font-semibold
- **不准** `bg-morandi-gold` 純色（金色 = 注目色、應該給按鈕用、不該給 status）

### 禁止 pattern（PR 會被打回）

```tsx
// ❌ 自己手刻 className
<span className="text-xs px-2 py-0.5 rounded-full bg-morandi-red/10 text-morandi-red">已取消</span>

// ❌ 用 <Badge> 直接傳業務色
<Badge className="bg-morandi-gold text-white">待處理</Badge>

// ❌ component 內自己宣告 statusMap / statusColors
const statusMap = { pending: { style: 'bg-morandi-X/20 text-morandi-X' } }
```
